...
frame_number = 13
video_name = "video1"
filename = "{}-frame-{}.png".format(video_name, frame_number)

# Leo las imagenes de entrenamiento
imNp = imread(
    "D:/proyectos/robotics/assignment-2/classify/training/images/originals/{}".format(filename))
markImg = imread(
    "D:/proyectos/robotics/assignment-2/classify/training/images/sections/{}".format(filename))
...
ax.plot(data_marca[:, 0], data_marca[:, 1],
        data_marca[:, 2], 'r.', label='marca')
ax.plot(data_fondo[:, 0], data_fondo[:, 1],
        data_fondo[:, 2], 'g.', label='fondo')
ax.plot(data_linea[:, 0], data_linea[:, 1],
        data_linea[:, 2], 'b.', label='linea')
