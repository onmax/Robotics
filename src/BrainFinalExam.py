...
def write_text(img, texts):
    ...
    text_width, text_height = max([s[0] for s in texts_sizes]), sum(
        [s[1] for s in texts_sizes]) + 10 * (len(texts) - 1)
    ...
    for i, text in enumerate(texts):
        padding_top = 0 if i == 0 else sum(
            s[1] for s in texts_sizes[:i]) + 10 * i
        img = cv2.putText(img, text, (padding, padding + 15 + padding_top),
                          font, fontScale=font_scale, color=color, thickness=1)
...
class SceneMoments():
    ...
    def get_defects(self):
        chull_list = [cv2.convexHull(contour, returnPoints=False)
                      for contour in self.contours]
        defects = [cv2.convexityDefects(contour, chull) for (
            contour, chull) in zip(self.contours, chull_list)]
        ...
        defect = [[start, end, mid, length] for start, end, mid,
                  length in defect if length > self.min_contour_size]
        return defect
    ...
    def sstr(self):
        ...
        if not self.compl:
            lengths = "{} contours: {}  defects: {}".format(
                self.type_object, len(self.contours), len(self.defects))
            return [lengths, ""]
        else:
            lengths = "{} contours: {}  defects: {}".format(
                self.type_object, len(self.contours), len(self.defects))
            lengths_compl = "Compl contours: {}  defects: {}".format(
                len(self.contours_compl), len(self.defects_compl))
    ...
    def paint_defects(self, img, color):
        ...
        for s, e, m, l in self.defects:
            cv2.circle(img, (contour[m][0][0], contour[m]
                             [0][1] + top_offset), 5, color, -1)
        return img
    ...
    def paint_lines(self, img, color):
        ...
        contour = self.contour
        for s, e, m, l in self.defects:
            cv2.line(img, (contour[s][0][0], contour[s][0][1] + top_offset),
                     (contour[e][0][0], contour[e][0][1] + top_offset), color, 2)
        return img
    ...
    def paint_contours(self, img, color):
        if len(self.contours) == 0:
            return img
        contour = self.contour
        contour = np.array([[[c[0][0], c[0][1] + top_offset]]
                            for c in contour])
...
class EuclidianClassifier():
    ...
    def get_pixels(self, data):
        ...
        for (original, labels) in data:
            ...
            signs = signs + \
                [v for v in normalized[np.all(labels == [255, 0, 0], 2)]]
            bck = bck + \
                [v for v in normalized[np.all(labels == [0, 255, 0], 2)]]
            line = line + \
                [v for v in normalized[np.all(labels == [0, 0, 255], 2)]]
...
class SceneDescription():
    ...
    def active_lane_from_memory(self, memory):
        m100 = [m.description.boundaries.get_active_lane()
                for m in memory[-60:-10]]
        mean = np.mean(np.array([b.mid[0] for b in m100 if b]))
        closest = np.abs([b.mid[0]
                          for b in self.boundaries.bottom] - mean).argmin()
    ...
    def get_small_image(self, w, h):
        ...
        self.h1, self.h2 = int(
            self.image.shape[0]) - h, int(self.image.shape[0])
