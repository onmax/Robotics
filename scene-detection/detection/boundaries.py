...
class Boundaries:
    ...
    def get_paths_boundaries(self, v):
        ...
        p = list(zip(v[:-1], v[1:]))
        start = [np.where(np.all(np.all(p == np.array([g,b]), axis=2), axis=1))[0] + 1][0]
        end = [np.where(np.all(np.all(p == np.array([b,g]), axis=2), axis=1))[0]][0]
        if np.all(v[0] == b):
            start = np.insert(start, 0, 0)
        if np.all(v[-1] == b):
            end = np.insert(end, len(end), len(v) - 1)
        mid = np.array([int(np.mean(x)) for x in np.array(list(zip(start, end)))])
    ...
    def set_boundaries(self, sections_img):
        ...
        heigth, width = img.shape[0] - 1, img.shape[1] - 1
        top = [Boundary(coords, x=0) for coords in self.get_paths_boundaries(img[0,:])]
        bottom = [Boundary(coords, x=heigth) for coords in self.get_paths_boundaries(img[-1,:])]
        right = [Boundary(coords, y=width) for coords in self.get_paths_boundaries(img[:,-1])]
        left = [Boundary(coords, y=0) for coords in self.get_paths_boundaries(img[:,0])]
        
        self.top, self.bottom, self.right, self.left = self.remove_one_in_the_corners(top, bottom, right, left)

        self.total = len(self.top) + len(self.bottom) + len(self.right) + len(self.left)
